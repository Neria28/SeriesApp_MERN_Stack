...
export default function AddSerieComp() {
  ...
  const sendToDB = async () => {
    let resp = await requests.post("http://localhost:8080/api/series", newSerie);
  ...
  return (
    <div>
      <form>
        ...
        <br />
        <label>Genres </label>
        <input type="text" onChange={(e) => putGenres(e.target.value)} />
        <br />
        <label>Image Url </label>
        <input
          type="text"
          onChange={(e) => setNewSerie({ ...newSerie, img: e.target.value })}
        />
        <br />
        ...
        <input
          type="date"
          onChange={(e) =>
            setNewSerie({ ...newSerie, premiered: e.target.value })
          }
        />
        <br />
        <input type="button" value="Save" onClick={() => sendToDB()} />
        <input
          type="button"
          value="Cancel"
